...
### Added
- **`/health` reports whether the service can write its data** — a new `data_writable` flag, and `status` becomes `degraded` when `ean-db.json` or its directory is not writable; localhost clients also get the offending paths in `unwritable_paths`. A git reset run as root in the server checkout left `ean-db.json` owned by root, and every `PUT /api/ean` failed with a 500 for a month while `/health` said "ok".
- **`/health` reports whether the deployment's self-update is stuck** — a new `update` block carries the checkout's revision, the revision the virtualenv was last successfully installed at, a consecutive-failure count and (for localhost clients only, since pip and git quote filesystem paths) the last error, and `status` becomes `degraded` when the two revisions disagree, a run failed, or no run has happened for as long as the deployment says it should take. A failed update is otherwise invisible: `tingbok-update` on the VM pulls, reinstalls and restarts on a timer, but when a step fails the running service keeps answering from the code it imported at startup, so nothing looks wrong until the next restart — which is how a stranded checkout once went unnoticed for seven weeks. `scripts/update_status.py` maintains the file, `TINGBOK_UPDATE_STATUS_FILE` points at it, and a checkout with the variable unset reports no `update` block, so nothing changes off a managed host.
...
### Fixed
- **Wikidata rate limiting (HTTP 429) is honoured.** After a 429, tingbok pauses all Wikidata requests for the Retry-After period (default 60 s, at most an hour) instead of retrying straight away. A failed Wikidata or DBpedia request is no longer cached as an answer: before, a 429 or a network error could be stored as "this concept has no labels / aliases / description", or let a person or place through the concept filter, for the whole cache lifetime. Every Wikidata request now carries a descriptive User-Agent, as Wikimedia's policy asks.
